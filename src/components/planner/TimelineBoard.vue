...
<template>
    <AppPanel tone="muted" class="space-y-6">
        ...
        <AppEmptyState v-else-if="items.length === 0" title="No items in the timeline yet :c" description="Add the first asset or milestone above :3" />

        <div v-else class="overflow-x-auto">
            <div class="min-w-80 space-y-2">
                <div class="grid items-center text-xs uppercase text-(--app-text-muted)" :style="{ gridTemplateColumns: timelineGridTemplate }">
                    <div class="sticky left-0 z-20 bg-(--app-surface-muted) px-3 py-2">Item</div>
                    <div v-for="day in timelineDays" :key="day.toISOString()" class="border border-(--app-border) px-2 py-2">{{ formatTimelineDay(day) }}</div>
                </div>
            </div>

            <div v-for="item in items" :key="item.id" class="grid min-h-16 items-center rounded-xl border border-(--app-border) bg-(--app-bg-subtle)" :style="{ gridTemplateColumns: timelineGridTemplate }">
                <div class="sticky left-0 z-20 h-full bg-(--app-bg-subtle) px-3 py-3">
                    <p class="text-sm font-bold text-(--app-text)">{{ item.title }}</p>
                    <p class="mt-1 text-xs text-(--app-text-muted)">{{ item.item_type }}
                ...
                <div v-for="day in timelineDays" :key="`${item.id}-${day.toISOString()}`" class="h-full border border-(--app-border)" />

                <div class="z-10 mx-1 rounded-lg px-3 py-2 text-xs font-bold text-black" :class="item.item_type === 'milestone' ? 'aspect-square w-10 rotate-45 justify-self-center' : ''" :style="timelineBarStyle(item)">
                ...
                <div class="col-start-1 -col-end-1 px-3 py-3">
                    <TimelineItemForm v-if="editingItemId === item.id" :item="item" :collaborators="collaborators" :loading="updatingItemId === item.id" @submit="emit('save', item, $event)" @cancel="emit('cancelEdit')" />

                    <div v-else class="flex flex-wrap items-center gap-3">
                        <AppSelect :model-value="item.status" :options="statusOptions" class="max-w-40" :disabled="updatingItemId === item.id" @update:model-value="emit('statusChange', item, $event as PlannerTimelineStatus)" />

                        <AppButton type="button" variant="ghost" size="sm" :disabled="updatingItemId === item.id" @click="emit('shift', item, -1)">Move -1 day</AppButton>
                        <AppButton type="button" variant="ghost" size="sm" :disabled="updatingItemId === item.id" @click="emit('shift', item, 1)">Move 1 day</AppButton>
                        <AppButton v-if="item.item_type !== 'milestone'" type="button" variant="ghost" size="sm" :disabled="updatingItemId === item.id" @click="emit('extend', item, 1)">Extend +1 day</AppButton>
                        <AppButton type="button" variant="ghost" size="sm" :disabled="updatingItemId === item.id" @click="emit('edit', item)">Edit</AppButton>
                        <AppButton type="button" variant="ghost" size="sm" :disabled="deletingItemId === item.id" @click="emit('remove', item.id)">{{ deletingItemId === item.id ? 'Removing...' : 'Remove' }}</AppButton>
