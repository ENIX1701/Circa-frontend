...
<template>
    <AppPanel class="space-y-4">
        ...
        <AppEmptyState v-if="items.length === 0" title="Everything has an owner!" description="Suspiciously responsible behavior..." />

        <div v-else class="space-y-3">
            <article v-for="item in items" :key="item.id" class="roudned-xl border border-(--app-border) bg-(--app-bg-subtle) p-4">
