...
import { type AddEventCollaboratorRequest, type EventCollaboratorRecord, type EventMembershipRole, useEvents } from '@/composables/useEvents';
...
<template>
    <div class="space-y-8">
        <AppPageHeader eyebrow="Collaborators" title="Event team" description="Manage who can access and edit this event! :3" />
        ...
        <AppAlert v-else tone="info">
            Only event owners can manage collaborator roles >:c
        </AppAlert>
        ...
        <AppPanel tone="muted" class="space-y-6">
            ...
            <AppEmptyState v-else-if="collaborators.length === 0" title="No collaborators yet QwQ" description="It's always more fun with friends..." />

            <div v-else class="space-y-3">
                <CollaboratorRow v-for="collaborator in collaborators" :key="collaborator.user_id" :member="collaborator" :can-manage="isOwner" :updating="updatingUserId === collaborator.user_id" :deleting="deletingUserId === collaborator.user_id" @role-change="handleRoleChange" @remove="handleRemoveCollaborator" />
            </div>
        </AppPanel>
    </div>
</template>
